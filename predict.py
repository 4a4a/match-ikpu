...
def save_embeddings(file, texts, vectors, codes, names, checksums):
    ...
    pd.DataFrame({"ИКПУ": codes, "text": texts, "checksum": checksums}).to_csv(file.replace('.npz', '_checksums.csv'), index=False)
...
def find_best_match(query_text, embeddings, ikpu_df):
...
try:
    ...
    feedback['query'] = (
        feedback['Название'].astype(str) + " " +
        feedback['Категория'].astype(str) + " " +
        feedback['brand'].astype(str)
    ).apply(preprocess_text)
    ...
    print(f"📌 Добавлено из feedback: {len(feedback)} записей")
...
if validate_cache(CATALOG_CHECKSUM_FILE, catalog_checksums):
    ...
    save_embeddings(CATALOG_EMBED_FILE, catalog['text'].tolist(), np.array(catalog_vecs), catalog['ИКПУ'].tolist(), catalog['Название ИКПУ'].tolist(), catalog_checksums)
...
for _, row in tqdm(products.iterrows(), total=len(products), desc="🔍 Обработка товаров"):
    best_row, best_score = find_best_match(row['query'], all_embeddings, all_ikpu)
    results.append({
        "ID": row["ID"],
...
# === Экспорт ===
pd.DataFrame(results).to_excel("data/predicted_ikpu.xlsx", index=False)
